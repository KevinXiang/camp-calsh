...
export class EffectManager {
    constructor(scene) {
        this.scene = scene;
        this.budget = new EffectBudget(50);
    }
    ...
    dispatch(events) {
        for (const ev of events) {
            switch (ev.kind) {
                case 'meleeHit':
                    this.spawnMeleeStars(ev.x, ev.y);
                    break;
                case 'unitDeath':
                    this.spawnDeathStars(ev.x, ev.y);
                    break;
                case 'campHit':
                    this.shakeCamera();
                    break;
                case 'campDestroyed':
                    this.spawnCampDestroy(ev.x, ev.y);
                    break;
            }
        }
    }
    ...
    spawnCampDestroy(x, y) {
        if (!this.budget.tryAdd())
            return;
        const root = this.scene.add.container(x, y);
        // 烟雾圈
        const smokes = [[0, 0, 0], [-15, -10, 100], [18, -5, 200]];
