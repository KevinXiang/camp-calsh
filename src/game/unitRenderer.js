...
function drawStickFigure(g, faction, kind) {
...
function drawWeapon(g, kind, color) {
    switch (kind) {
        case 'sword': {
...
export function drawUnit(scene, unit) {
...
export function updateUnitView(view, unit) {
    view.setPosition(unit.x, unit.y);
    // 死亡处理：只在第一帧切换为尸体并锁定
    if (!unit.alive) {
        if (view.getData('corpse') !== true) {
            // 第一次进入死亡：先播倒下旋转 → 再切尸体
            const body = view.getData('body');
            if (body) {
 ...
 */
export function maybeTriggerAttackAnim(view, unit) {
    const anim = view.getData('anim');
    const body = view.getData('body');
    if (!anim || !body || !unit.alive)
        return;
    ...
    switch (anim.kind) {
        ...
        case 'archer':
            ...
            break;
        case 'javelin':
            playJavelinAnim(body);
            break;
    }
}
/** 剑兵挥砍：body 旋转挥砍（yoyo 回到 0，不与 walk 冲突） */
function playSlashAnim(body) {
 ...
 */
export function triggerHitFlash(view) {
    const body = view.getData('body');
    if (!body)
        return;
