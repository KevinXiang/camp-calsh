...
**整体风格（PRD 16.1）**：Q版火柴人 + 低暴力 + 伪 2.5D + 卡通炫酷。面向 10 岁男孩：小兵为火柴人造型（肤色圆头 + 微笑表情 + 阵营色身体线条）；武器像玩具（金剑 / 银盾 / 绿弓 / 橙矛），不强调真实杀伤，无血腥。红方烈焰红橙黄 / 蓝方电光蓝亮青，对比鲜明。
...
- 阵营区分：身体线条主色（红/蓝）+ 武器色区分兵种
- **渲染（阶段 2 已实现）**：Phaser Graphics 绘制火柴人（肤色圆头 + 阵营色肢体 + 武器造型 + 脚底阴影 + 头顶圆角血条），零外部资源依赖
...
**军营外观（PRD 16.3）**：
- 积木城堡 + 红蓝三角旗 + 城垛 + 门洞 + 落影；4 种类型用装饰条颜色区分
- **渲染（阶段 1 已实现）**：Phaser Graphics 绘制积木城堡（圆角主体 + 城垛 + 门洞 + 三角旗 + 兵种装饰色条），零外部资源依赖
- 摧毁 → 积木废墟表现
